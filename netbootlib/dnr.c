...
#if !defined (__GESTALT__) && !defined (THINK_C)
#include <Gestalt.h>
#else
...
OSErr OpenResolver(char *fileName)
{
	...
	rc = CallOpenResolverProc((OpenResolverProcPtr)gDNRCodePtr, OPENRESOLVER, fileName);
	if (rc != noErr) 
	{
...
OSErr CloseResolver(void)
{
	...
	CallCloseResolverProc((CloseResolverProcPtr)gDNRCodePtr, CLOSERESOLVER);
			...
			ResultUPP resultproc, Ptr userDataPtr)
{
	...
	return (CallStrToAddrProc((StrToAddrProcPtr)gDNRCodePtr, STRTOADDR, hostName, rtnStruct, resultproc, userDataPtr));
}
...
OSErr AddrToStr(unsigned long addr, char *addrStr)
{
	...
	err = CallAddrToStrProc((AddrToStrProcPtr)gDNRCodePtr, ADDRTOSTR, addr, addrStr);
	return(noErr);
}
...
OSErr EnumCache(EnumResultUPP resultproc, Ptr userDataPtr)
{
	...
	return (CallEnumCacheProc((EnumCacheProcPtr)gDNRCodePtr, ENUMCACHE, resultproc, userDataPtr));
}
			...
			ResultUPP resultproc, Ptr userDataPtr)
{
	...
	return(CallAddrToNameProc((AddrToNameProcPtr)gDNRCodePtr, ADDRTONAME, addr, rtnStruct, resultproc, userDataPtr));
}
			...
			ResultProc2UPP resultProc, Ptr userDataPtr)
{
	...
	return(CallHInfoProc((HInfoProcPtr)gDNRCodePtr, HXINFO, hostName, returnRecPtr, resultProc, userDataPtr));
			...
			ResultProc2UPP resultProc, Ptr userDataPtr)
{
	...
	return(CallMXInfoProc((MXInfoProcPtr)gDNRCodePtr, MXINFO, hostName, returnRecPtr, resultProc, userDataPtr));
